...
describe('<FrontPage />', () => {
  ...
  it('Should fetch "FrontPageDetails" on load', async () => {
    let props = {};
    props.location = {};
    props.location.pathname = '/page/1';
    props.history = {};
    props.history.push = jest.fn();
    props.history.listen = jest.fn();
    ...
    const node = [{ clientWidth: 200 }];
    jest.spyOn(document, 'getElementsByClassName').mockImplementation(() => node);
    jest.spyOn(window, 'addEventListener').mockImplementation(() => jest.fn());
    const getFrontPageDetails = jest
      .spyOn(NewsService, 'getFrontPageDetails')
      .mockImplementation(() => Promise.resolve(response));

    await act(async () => {
      const { asFragment } = render(<FrontPage {...props} />);
      expect(asFragment()).toMatchSnapshot();
    });
    expect(getFrontPageDetails).toBeCalledTimes(1);
  });
});
